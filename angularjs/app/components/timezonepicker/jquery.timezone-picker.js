  ...
  var GoogleMapsMapper = function(el, mouseClickHandler, mouseMoveHandler, mapOptions) {
    ...
    var showInfoWindow = function(pos, content, callback) {
      ...
      gmaps.event.addListener(infowindow, 'domready', function() {
        ...
        $('img[src="https://maps.gstatic.com/mapfiles/api-3/images/mapcnt3.png"]').each(function() {
            $(this).attr('width', 14);
            $(this).attr('height', 13);
            $(this).attr('top', 0);
            $(this).attr('left', 0);
            $(this).css({width: '14px', height: '13px', top: '0px', left: '0px'});
  ...
  selectPolygonZone = function(polygon) {
    ...
    if (_options.onSelected) {
      ...
      showInfoWindow('<h2>' +
        ...
        pad(adjusted.getSeconds()) + '</p>' +
        '<p>Your Time: ' +
        pad(now.getHours()) + ':' +
        pad(now.getMinutes()) + ':' +
        pad(now.getSeconds()) + '</p>' +
        '<p>UTC Offset (in hours): ' +
        (utcOffset / 60) + '</p>');
    }
  };
