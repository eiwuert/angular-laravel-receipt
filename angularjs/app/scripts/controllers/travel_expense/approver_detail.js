rciSpaApp.controller('ApproverDetailCtrl', ['$scope', '$timeout', '$rootScope', '$route', 'Restangular', '$location' , '$templateCache', function($scope, $timeout, $rootScope, $route, Restangular, $location, $templateCache){
    ...
    $scope.$on('$routeChangeSuccess', function(event, currentRoute, previousRoute) {
        if (currentRoute.currentPath === '/travel-approver' && currentRoute.params.hasOwnProperty('reportId')) {
    ...
    $scope.loadReportDetail = function(reportId) {
        $scope.$emit('LOAD_TA_DETAIL', reportId)
    }

    /**
     * Listen to load report detail
     */
    $rootScope.$on('LOAD_TA_DETAIL', function(event, reportId, reportType, dateFrom, dateTo, tripInfo) {
        jQuery('.page-app').hide();
        jQuery('#ngview-wrapper').hide();
        jQuery('#approver-detail-wrapper').show();

        //Recalculate to make columns Submitter and Approver have an equal width
        $('#app-ta .tb-rl  .col-apr').equalWidth('#app-ta .tb-rl .col-sub');
...
//                $scope.report.Amount += parseFloat(trip.Amount);
            ...
            truncateReportDetailTableText('ta', 'report');
        }
...
//        var totalItem   = trip.Items.length;
    ...
    $scope.triggerTruncateFunction = function() {
        $timeout(function(){
            truncateReportDetailTableText('ta', 'cat');
        }, 200);
    }
