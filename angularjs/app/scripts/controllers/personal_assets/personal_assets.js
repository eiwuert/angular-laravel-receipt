...
rciSpaApp.controller('PACtrl', function($scope, $rootScope, Restangular, $route, $location, localStorageService, $timeout) {
    /*Object click status*/
    $scope.clicked = {
        periodRange: false,
        listItemOnly: false,
    ...
    $scope.paItems = $scope.PACats = $scope.tmpPaList = [];

    var paData = [];
    var paCats = [];
    var step = 0;
    var intervalPALoad;
    $scope.loadMorePa = function() {
        if (paData.length && paData[0].length > step) {
            var rows = (paData[0].slice(step, step + 5));
            $scope.$apply(function() {
                $scope.PACats = $scope.PACats.concat(rows);
                $scope.tmpPaList = $scope.PACats;
            });
            step += 5;
        }
        if (angular.isDefined(paData[0]) && paData[0].length == $scope.PACats.length) {
            clearInterval(intervalPALoad);
        }
    }
    ...
    $scope.getPAList = function(reload) {
        if (localStorageService.isSupported()) {
            var userPA = localStorageService.get('userPA');

            if (userPA && !reload) {
                paData = angular.fromJson(userPA);
                $scope.paItems = paData[1];
                $scope.totalAmount = paData[2];

                $rootScope.loadedModule++;

                if (typeof paData[3] !== 'undefined') {
                    var paPeriods = paData[3];
                    $scope.usePeriodRange = paData[3].usePeriodRange;
                    $scope.periodFrom = paData[3].dateFrom;
                    $scope.periodTo = paData[3].dateTo;
        ...
        Restangular.one('categories').customGET('',
                ...
                .then(function(response) {
                    paData = arrangeToMultiLevelCats(response);
                    paData.push({
                    ...
                    if (localStorageService.isSupported()) {
                        localStorageService.add('userPA', angular.toJson(paData));
                    }
    ...
    $scope.deleteItems = function() {
        var itemDeleteList = [];
        $('#pa-item-list tbody tr[class^="cat-lv"].item .app-icon.icon-checkedbox-sqr').each(function(k, v) {
        ...
        $.showMessageBox({
            content: '<p>Are you sure you want to remove selected item(s) from PersonalAssets?</p><p>Please note, the receipt/invoice relating to this item will not be removed from your ReceiptBox.</p>',
    ...
    $scope.export = function() {
        ...
        var contentMessage = "Export PersonalAssets data for the period of<br>";
        if ((typeof ($scope.periodTo) == 'undefined') || ($scope.periodFrom == $scope.periodTo)) {
            ...
            title = "PersonalAssets in " + m_names[monthFrom] + " " + yearFrom;
        } else {
            ...
            title = "PersonalAssets from " + m_names[monthFrom] + " " + yearFrom + " to " + m_names[monthTo] + " " + yearTo;
        }
    ...
    $scope.toggleRange = function() {
        ...
        if (typeof ($scope.tmpUsePeriodRange) == 'undefined' || !$scope.tmpUsePeriodRange) {
            $('#personal-assets-wrapper .filterdate .date-to .ui-datepicker-month').trigger('change');
        ...
        if(app == 'personal-assets'){
            if(dataPertype){
                ...
                $timeout(function(){
                    ...
                    if(dataPertype.mountPA == 1){
                        var datefrom = defaultDate.getFullYear() + '-' + ('0' + (defaultDate.getMonth() + 1)).slice(-2) + '-01';
                        $location.path('/personal-assets/analytic/'+datefrom+'/');
                    }else if(dataPertype.mountPA == 3){
                        ...
                        $location.path('/personal-assets/analytic/'+datefrom+'/'+dateto+'/');
                    }else{
                        var datefrom = defaultDate.getFullYear() + '-' + ('0' + (defaultDate.getMonth() + 1)).slice(-2) + '-01';
                        $location.path('/personal-assets/analytic/'+datefrom+'/');
                        $timeout(function(){
                    ...
                    $scope.getPAList(true);
                });
            }
        }
    });
    // Listener for updating PA local storage
    $rootScope.$on("UPDATE_PA_LOCAL_STORAGE", function(event, message) {
        try {
            var defaultDate = new timezoneJS.Date(new Date(), $rootScope.loggedInUser.Timezone);
        } catch (err) {
            var defaultDate = new Date();
        ...
        Restangular.one('categories').customGET('',
            ...
            var paData = arrangeToMultiLevelCats(response);

            if (localStorageService.isSupported()) {
                localStorageService.add('userPA', JSON.stringify(paData));
            }
    ...
    $rootScope.$on('RELOAD_PA_LIST', function(event, loadFromServer) {
        $scope.getPAList(loadFromServer);
    });
});
